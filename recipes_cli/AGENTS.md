...
`USAGE.md` (at the repo root) documents the `recipes` CLI's full syntax and output. It is generated by `uvx showboat` and must be regenerated whenever CLI syntax or semantics change — new subcommands, renamed options, altered output, etc.
