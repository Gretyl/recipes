...
CLI changes can affect two files outside this package. Update both whenever subcommands, options, or output semantics change.

### USAGE.md

`USAGE.md` (repo root) documents the `recipes` CLI's full syntax and output. It is generated by `uvx showboat` and must be regenerated whenever CLI syntax or semantics change — new subcommands, renamed options, altered output, etc.

After making CLI changes, run `uvx showboat --help` and use showboat to update `USAGE.md`.

### AGENTS.md (top-level)

The top-level `AGENTS.md` cross-references CLI subcommands in two places:
